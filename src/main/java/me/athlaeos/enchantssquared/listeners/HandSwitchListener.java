...
public class HandSwitchListener implements Listener {
    ...
    public void onHotbarSwitch(PlayerItemHeldEvent e){
        ...
        if (CooldownManager.getInstance().isCooldownPassed(e.getPlayer().getUniqueId(), "hand_equipment_reset")){
            CooldownManager.getInstance().setCooldown(e.getPlayer().getUniqueId(), 500, "hand_equipment_reset");
    ...
    public void onInteract(PlayerInteractEvent e){
        ...
        if (playersWhoSwitchedItems.remove(e.getPlayer().getUniqueId())){
            EntityEquipmentCacheManager.getInstance().resetHands(e.getPlayer());
        }
    }
}